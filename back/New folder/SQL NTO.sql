...
9)Send a db or tables in a db to a third party. 
mysqldump -uroot -p  DB_name table1 table2> pilePath
NOTE!! NO comma(,) between tables!!
e.g. mysql -uroot -p mydb < C:\xampp\htdocs\New folder\mySQL-forPDO2.sql

 *If a file path includes any space, the pile path should be surrounded with ""!!
  e.g mysqldump -uroot -p  mydb product order_items> "C:\xampp\htdocs\New folder\mySQL -for PDO 2.sql"



10)2 ways to change columns of a table!!
 (1)DROP a table; => Recreate a table with different cols. 
 (2)ALTER TABLE table1 ADD/DROP COLUMN col1; 